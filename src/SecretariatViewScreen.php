...
// retrieve secretariat's id from the URL
...
$sql_edited_files= "SELECT `id`, `committee`, `topic`, `resolution_filename`, `status` FROM resolutions ORDER BY `id`";
$result= mysqli_query($cnx, $sql_edited_files)

or die("SELECT Error: ".mysqli_error($cnx));


// draw table with only the "edited" resolutions

print "<table id='secretariattbl' border=1>\n";
...
echo "<tr>\n";
echo "<td>$res_id_value</td>";
echo "<td>$committeevalue</td>";
echo "<td>$topicvalue</td>";
echo "<td><div align=center>$files_field</div></td>";
echo "<td><div align=center><form action='SecretariatProcessView.php?id=$res_id_value&$found_id' method='post' enctype='multipart/form-data'><input type='submit' name='submit' value='Download File'/></form></div></td>";
echo "</tr>\n";
   }
}
...
<body>
  <h2>Edited Resolutions to be approved</h2>
...
function sortTable(n) {
  ...
  table = document.getElementById("secretariattbl");
  switching = true;
